...
namespace Macho.CodeEditor
{
        ...
        : GH_Component(name, nickname, description, Config.Category, Config.SubCat.Operation)
    {
        ...
        {
            Attributes = new CodeEditorAttributes(this);
        }

        /// <summary>
        /// Extensions we can resolve to assemblies
        /// </summary>
        private static string[] dll_extensions =
        [
            ".dll",
            ".gha",
            ".ghc"
        ...
        private static string[] ignored_folders =
        [
            "bin", "obj"
        ];
        ...
        /// <summary>
        /// If true, the Read method has been called.  Used to determine if the user has 
        /// placed the component or whether it is loaded/pasted/etc.
        /// </summary>
        ...
        public sealed override bool Read(GH_IReader reader)
        {
            ...
            if (this is CodeEditorPlaceholder)
            {
        ...
        /// </summary>
        private void ReadNestedChunk(CodeEditorBase component)
        {
            if (m_chunk != null)
            {
        ...
        {
            return Directory.CreateDirectory(Path.Combine(WorkingDir, "Source", Guid.NewGuid().ToString())).FullName;
        }
        ...
        {
            return Directory.CreateDirectory(Path.Combine(WorkingDir, "Builds", Guid.NewGuid().ToString())).FullName;
        }
        ...
        {
            Debug.Assert(this is CodeEditorPlaceholder, "New projects should only be generated for placeholder components. Existing components should instantiate the existing source code.");
        ...
        /// <summary>
        /// Extracts the project template (where .gha is located) to a folder
        /// </summary>
        ...
        {
            Assembly assembly = Assembly.GetAssembly(typeof(CodeEditorBase));
            if (assembly == null)
            {
            ...
            string templateDir = Path.Combine(assemblyDir, "Template");
            if (!Directory.Exists(templateDir))
            {
                throw new DirectoryNotFoundException("Unable to locate template directory.");
            }

            CopyDirectoryRecursive(templateDir, destinationFolder, path => Path.GetFileName(path) is { } name &&
        ...
        /// </summary>
        internal void EditSourceProject()
        {
            if (m_sourcePath == null)
            {
                if (!(this is CodeEditorPlaceholder))
        ...
        private void CacheSource(string folder)
        {
            ...
            DirectoryInfo workingFolder = new DirectoryInfo(Path.Combine(WorkingDir, "Cache"));
            ...
            string tmpZipFile = Path.Combine(WorkingDir, "tmp_source_cache.zip");
            if (File.Exists(tmpZipFile))
            {
                File.Delete(tmpZipFile);
            }
        ...
        private void CacheBinaries(string folder)
        {
            ...
            DirectoryInfo workingFolder = new DirectoryInfo(Path.Combine(WorkingDir, "Cache2"));
            if (workingFolder.Exists)
            {
                workingFolder.Delete(true);
            }
            ...
            string tmpZipFile = Path.Combine(WorkingDir, "tmp_binary_cache.zip");
            if (File.Exists(tmpZipFile))
            {
                File.Delete(tmpZipFile);
            }
        ...
        private string RestoreBinaries()
        {
            ...
            string tmpZipFile = Path.Combine(WorkingDir, "tmp_binary_restore.zip");
            Directory.CreateDirectory(Path.GetDirectoryName(tmpZipFile));
            if (File.Exists(tmpZipFile))
            {
                File.Delete(tmpZipFile);
        ...
        /// <returns></returns>
        private CodeEditorBase LoadComponentFromFilePath(string path)
        {
            using (FileStream fs = File.OpenRead(path))
            {
        ...
        private CodeEditorBase LoadComponentFromStream(Stream stream)
        {
            using (MemoryStream ms = new MemoryStream())
            {
                stream.CopyTo(ms);
        ...
        private CodeEditorBase LoadComponentFromByteArray(byte[] data)
        {
            Assembly assm = Assembly.Load(data);

            Type[] componentTypes = assm.GetExportedTypes().Where(
                type => typeof(CodeEditorBase).IsAssignableFrom(type)
                ).ToArray();
            ...
            CodeEditorBase inst = Activator.CreateInstance(componentTypes[0]) as CodeEditorBase;
            return inst;
        }
        ...
        {
            CodeEditorPlaceholder newComponent = new CodeEditorPlaceholder
            {
        ...
        private void ReplaceComponent(CodeEditorBase newComponent)
        {
            ...
            newComponent.m_hasDeserialized = true;
